...
import { FFProgram } from "../../FFProgram.sol";
...
type FFProgramType is bytes32;

// equivalent to WorldResourceIdLib.encode({ typeId: RESOURCE_SYSTEM, namespace: "GardenOfEden", name: "FFProgram" }))
FFProgramType constant fFProgram = FFProgramType.wrap(
  0x737947617264656e4f664564656e0000464650726f6772616d00000000000000
);
 ...
 */
library FFProgramLib {
  error FFProgramLib_CallingFromRootSystem();
  ...
  function onHit(FFProgramType self, HookContext memory ctx, IHit.HitData memory hit) internal {
  ...
  function onBuild(FFProgramType self, HookContext memory ctx, IBuild.BuildData memory build) internal {
    return CallWrapper(self.toResourceId(), address(0)).onBuild(ctx, build);
  }

  function onMine(FFProgramType self, HookContext memory ctx, IMine.MineData memory mine) internal view {
    return CallWrapper(self.toResourceId(), address(0)).onMine(ctx, mine);
  }

  function _msgSender(FFProgramType self) internal view returns (address __auxRet0) {
    return CallWrapper(self.toResourceId(), address(0))._msgSender();
  }

  function _msgValue(FFProgramType self) internal view returns (uint256 __auxRet0) {
  ...
  function callFrom(FFProgramType self, address from) internal pure returns (CallWrapper memory) {
    return CallWrapper(self.toResourceId(), from);
  }

  function callAsRoot(FFProgramType self) internal view returns (RootCallWrapper memory) {
    return RootCallWrapper(self.toResourceId(), WorldContextConsumerLib._msgSender());
  }

  function callAsRootFrom(FFProgramType self, address from) internal pure returns (RootCallWrapper memory) {
    return RootCallWrapper(self.toResourceId(), from);
  }

  function toResourceId(FFProgramType self) internal pure returns (ResourceId) {
    return ResourceId.wrap(FFProgramType.unwrap(self));
  }

  function fromResourceId(ResourceId resourceId) internal pure returns (FFProgramType) {
    return FFProgramType.wrap(resourceId.unwrap());
  }

  function getAddress(FFProgramType self) internal view returns (address) {
...
using FFProgramLib for FFProgramType global;
using FFProgramLib for CallWrapper global;
using FFProgramLib for RootCallWrapper global;
